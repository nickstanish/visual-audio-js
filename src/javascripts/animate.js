...
import * as AnalyserUtils from 'utils/analyser';
...
export function animate(mediaPlayer) {
  ...
  function drawBars() {
    ...
    const bufferLength = AnalyserUtils.getFrequencyBinCount(mediaPlayer.getAnalyser());
    const widthScale = 8.0;
    const barWidth = widthScale / bufferLength;
    const barPadding = barWidth * 0.2;

    const data = AnalyserUtils.getByteFrequencyData(mediaPlayer.getAnalyser());
  ...
  function drawScene() {
    ...
    if (mediaPlayer.isPlaying()) {
      const audioData = AnalyserUtils.getNormalizedFrequencyData(mediaPlayer.getAnalyser());
    ...
    if (options.blur) {
      ...
      let inVolume = 0;
      if (options.blur) {
        inVolume = AnalyserUtils.getNormalizedFrequencyData(mediaPlayer.getAnalyser()).average || 0;
      }
    ...
    if (mediaPlayer.isPlaying() && options.showBars) {
      drawBars();
    }
